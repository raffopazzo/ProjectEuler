...
/**
 * Return the british english spelling of the given number. 
 */
function number_to_letters(n) {
  if (n > 1000) throw n + " not supported yet";
  var words = { 0:  "zero",    1:  "one",       2:  "two",      3:  "three",
  ...
  if (n <= 20) {
    ...
    return words[10 * tens] + ((n % 10 > 0) ? number_to_letters(n % 10) : "");
  } else if (n < 1000) {
    var hundreds = Math.floor(n / 100);
    return number_to_letters(hundreds) + words[100]
         + ((n % 100 > 0) ? "and" + number_to_letters(n % 100) : "");
  } else {
    var thousands = Math.floor(n / 1000);
    return number_to_letters(thousands) + words[1000];
  }
}
